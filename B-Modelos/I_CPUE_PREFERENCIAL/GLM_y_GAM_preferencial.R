...
# Frecuentista ---------------------------
## GLM ---------------------------
modelo_f_r <- glm(CPUE ~ batimetria + batimetria_2 + tiempo, data = data_pref_final, family = Gamma(link="log"))
...
data_pred_glm <- data.frame(respuesta = as.vector(pred_glm), intervalo_lwr = as.vector(intervalo_lwr),
                            intervalo_upr = as.vector(intervalo_upr), tiempo = rep(1:10, each = 100))

## GAM ---------------------------
modelo_fgam_r <- gam(CPUE ~ s(batimetria) + te(xcoord,ycoord) + tiempo, data = data_pref_final, family = Gamma(link = "log"))
...
data_pred_gam <- data.frame(respuesta = as.vector(pred_gam), intervalo_lwr = as.vector(intervalo_lwr),
                            intervalo_upr = as.vector(intervalo_upr), tiempo = rep(1:10, each = 100))
...
psam <- sapply(r.samples, function(x) {
  ...
  predictor1 <- intercept + beta_2_pred*data_pref_final$batimetria[data_pref_final$tiempo == 1] + beta_3_pred*data_pref_final$batimetria_2[data_pref_final$tiempo == 1]
  
  pre=list();l=length(beta_y)
  for (i in 1:l){
    pre[[i]]=intercept + beta_y[i] + beta_2_pred*data_pref_final$batimetria[data_pref_final$tiempo == (i+1)] + beta_3_pred*data_pref_final$batimetria_2[data_pref_final$tiempo == (i+1)]
  }
...
modelo_gam_r_R2 <- bayesx(CPUE ~ sx(batimetria) + sx(xcoord, ycoord, bs = "te") + tiempo,
                          family = "Gamma", method = "MCMC", data = data_pref_final)
