...
# Mesh ---------------------------
loc <- cbind(data_pref_final$xcoord, data_pref_final$ycoord) # Localizaciones
...
data_pred_final$CPUE <- rep(NA, length(data_pred_final$batimetria)) # Anyadir biomasa

# INLA.GROUP ---------------------------
data_pred_final$batimetria[which(data_pred_final$batimetria > max(data_pref_final$batimetria))] <- NA # Mismos limites pred y est
data_pred_final$batimetria[which(data_pred_final$batimetria < min(data_pref_final$batimetria))] <- NA

all <- rbind(data_pref_final, data_pred_final) # Juntar pred y est
...
data_pref_final <- cbind(
  data_pref_final, allin$groupbath[1:length(data_pref_final$batimetria)],
  allin$igroupbath[1:length(data_pref_final$batimetria)]
) # Anyadir columnas inla.group

data_pred_final <- cbind(
  data_pred_final, allin$groupbath[(length(data_pref_final$batimetria) + 1):length(allin$batimetria)],
  allin$igroupbath[(length(data_pref_final$batimetria) + 1):length(allin$batimetria)]
) # Anyadir columnas inla.group

names(data_pref_final) <- c("batimetria", "xcoord", "ycoord", "CPUE", "tiempo", "IgBath", "Igroup")
names(data_pred_final) <- c("batimetria", "xcoord", "ycoord", "tiempo", "CPUE", "IgBath", "Igroup")
...
stack_est <- inla.stack(
  data = list(y = data_pref_final$CPUE, link = 1), # Variable respuesta
...
stack_pred <- inla.stack(
  data = list(y = data_pred_final$CPUE, link = 1), # Variable respuesta NA
